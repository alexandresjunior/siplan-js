export const buscarComites = async (definirCarregando, definirComites, definirTotalPaginas, definirTotalElementos, paginaAtual, tamanhoPagina, URL_API) => {
  ...
  try {
    ...
    const comitesMapeados = dados.content.map(comite => ({
      ...
      dtCriacao: comite.dtCriacao ? new Date(comite.dtCriacao).toLocaleDateString('pt-BR') : 'N/A',
      dtUltAtualizacao: comite.dtUltAtualizacao ? new Date(comite.dtUltAtualizacao).toLocaleDateString('pt-BR') : 'N/A'
    }));
...
export const atualizarComite = async (comiteAtualizado, URL_API_BASE) => {
    ...
    if (!resposta.ok) {
      const textoErro = await resposta.text();
      throw new Error(`Erro ao atualizar comitê: ${resposta.status} - ${textoErro}`);
    }
...
export const excluirComite = async (idComite, URL_API_BASE) => {
  ...
  if (!resposta.ok) {
    const textoErro = await resposta.text();
    throw new Error(`Erro ao excluir comitê: ${resposta.status} - ${textoErro}`);
  }
};