export const buscarConfiguracoes = async (
    ...
    try {
        ...
        const resposta = await fetch(`${URL_API}?tipoConfiguracao=${tipoConfiguracao}&page=${paginaAtual}&size=${tamanhoPagina}`, {
        ...
        const dadosMapeados = dados.content.map(item => ({
            ...
            ano: item.ano,
            dataFechamento: item.dataFechamento ? new Date(item.dataFechamento).toLocaleDateString('pt-BR') : 'N/A',
        ...
        console.error(`Erro ao carregar configurações (tipo ${tipoConfiguracao}):`, erro);
    } finally {
...
export const buscarCiclos = async (URL_CICLOS) => {
    const token = localStorage.getItem('token');
    const resposta = await fetch(URL_CICLOS, { headers: { 'Authorization': `Bearer ${token}` } });
...
export const buscarDiretorias = async (URL_DIRETORIAS) => {
    const token = localStorage.getItem('token');
    const resposta = await fetch(URL_DIRETORIAS, { headers: { 'Authorization': `Bearer ${token}` } });
